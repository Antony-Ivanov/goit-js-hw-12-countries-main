...
function onInput() {
  ...
  fetch(`https://restcountries.com/v3/name/${inputRef.value}`).then(response => {
    ...
    // если страна только одна, генерируем разметку с  данными и флагом
    if (data.length === 1) {
      return outputRef.insertAdjacentHTML('beforeend', data.map(({
        ...
        flag
         ...
         <li> Population: ${population}</li> <li>Languages: <ul class='markers-on-list'> ${languages.map(el => `<li>${el.name}</li>`).join('')}</li></ul></ul>
          <div class=right-part><img class= 'flag-img' src='${flag}'></div> </div>`));
    }

    if (data.length > 1 && data.length <= 10) {
      // если стран до 10 - генерируем список из 10 стран 
      return outputRef.insertAdjacentHTML('beforeend', `<ul class='markers-on-list'>${data.map(({
        name
      }) => `<li>${name}</li>`).join('')} </ul>`);
    }
    ...
    }); //если сопадений много  
  }).catch(err => (0, _core.error)({
    text: err
